 ...
 *
 * Input variable:  none
 * Global variable: interrupt_flag,acmp_value
 * Return type: none
 * Macros: HIGH_LEVEL, LOW_LEVEL
 *********************************************/
void LETIMER0_IRQHandler(void)
{
	INT_Disable();									//Disable interrupts
	interrupt_flag = LETIMER_IntGet(LETIMER0);		//Check if COMP0 and COMP1 interrupt
	if(interrupt_flag & LETIMER_IF_COMP0)			//check for COMP0 interrupt
	{
		...
		if(acmp_value)
		{
			if(acmpInit.vddLevel == LOW_LEVEL)							//Check for LOW threshold
			{
				...
				ACMP_ChannelSet(ACMP0, acmpChannelVDD, acmpChannel6);	//Set the channel
			...
			else
			{
				...
				ACMP_Init(ACMP0,&acmpInit);								//Initialize ACMP0
				ACMP_ChannelSet(ACMP0, acmpChannel6, acmpChannelVDD);	//Set channel
